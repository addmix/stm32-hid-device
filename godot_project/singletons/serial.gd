...
signal device_connected(port : String)
...
func _ready() -> void:
	...
	attempt_connection.call_deferred()
...
func attempt_connection() -> void:
	...
	#TODO - Verify that connected device is compatible, and responds to our commands
	# - if it doesn't respond to commands, exclude it and try the next device
	# - maybe this process could be narrowed by vendor/product ID?
	
	connected_port = serial.list_ports()[0].port_name
	
	device_connected.emit(serial.list_ports()[0].port_name)

func _on_disconnected(port : String) -> void:
	connected_port = ""
	device_disconnected.emit(port)
