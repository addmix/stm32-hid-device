...
u_int8_t nkro_report[NKRO_BYTES];
u_int8_t nkro_modifiers = 0;

bool isModifierKey(u_int8_t keycode) {
    return keycode >= 0xE0 and keycode <= 0xE7;
}

void pressKeyNKRO(u_int8_t keycode) {
  Serial.println(keycode);
  u_int8_t byteIndex = keycode / 8;
  u_int8_t bitIndex  = keycode % 8;
  nkro_report[byteIndex] |= (1 << bitIndex);


  if (isModifierKey(keycode)) {
    u_int8_t mod_bit = keycode - 0xE0;
    nkro_modifiers |= (1 << mod_bit);
  }
}

void releaseKeyNKRO(u_int8_t keycode) {
  u_int8_t byteIndex = keycode / 8;
  u_int8_t bitIndex  = keycode % 8;
  nkro_report[byteIndex] &= ~(1 << bitIndex);
...
void NKROReport(Adafruit_USBD_HID& usb_hid, u_int8_t report_id) {
  u_int8_t report[1 + NKRO_BYTES];       // 1 byte modifier + 13 bytes keys
